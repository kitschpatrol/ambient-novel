...
During a one-time content generation step, this JSON file goes through a number of transformations, where it's combined with audio files and ambient music tracks to yield a [revised JSON file](https://github.com/kitschpatrol/ambient-novel/blob/main/src/lib/data/book.json) with per-word timing data embedded in a `<span>` element's `data` attributes wrapping each word of the book. (The span elements are ugly, but this is a fast way to provide timing data to the front-end instead of maintaining a cleaner but higher-overhead JSON abstraction — though in hindsight the transformation from timing data to markup could have happened in a Svelte server function instead of the content generation script.)
